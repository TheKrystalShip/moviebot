...
- **The wish list is the one thing the bot writes down.** A film that is not on the tracker has no
  torrent to tag and no manifest to write, so a wish exists nowhere else. It goes in the state
  directory systemd hands the service, whole and through a rename, on every change.
